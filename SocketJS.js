var deployJavaWrapper = (function(o) {
  var deployJava,
    pending = "";
  (function() {
  ...
  o.set = function(attrName, value) {
    deployJava[attrName] = value;
  };
  o.exec = function(fnName, args) {
    if (typeof deployJava[fnName] !== 'function') return;
    var _write = document.write,
    ...
    deployJava[fnName].apply(deployJava, args);
    document.write = _write;
...
var SocketJS = {
  ...
  ready: false,
  queued: [],
  connect: function(args) { // {host:str,port:int,ssl:boolean,connect:callback,data:callback,disconnect:callback} TODO mode:auto|manual
    var host = args.host,
      port = args.port,
      ssl = args.ssl || false,
      ...
      data = args.data,
      disconnect = args.disconnect || null,
      fd = -1;
    if (typeof host === 'undefined' || typeof port === 'undefined' || typeof connect === 'undefined' || typeof data === 'undefined') return false;
    if (!this.ready) {
      this.queued.push(args);
      return true;
    }
    fd = this.engine.connect(host, port, ssl); /* , mode */
    if (fd !== -1) {
      // Success
      this.sockets[fd] = {connect: connect, data: data, disconnect: disconnect};
    }
    return true;
  },
  /* TODO scheduleread, schedulebinread etc */
  papply: {
    send: function(id) {
    ...
    sendline: function(id, str) {
      var nl = (typeof str === 'string')?str:"\r\n";
      return function(data) {
        SocketJS.send({
          id: id,
          data: data + nl,
  ...
  send: function(args) { // {id:int,data:string}
    var id = args.id,
      data = args.data;
    if (typeof id === 'undefined' || typeof data === 'undefined') return;
    this.engine.send(id, data);
  },
  close: function(id) {
    if (typeof this.sockets[id] === 'undefined') return;
  ...
  install: function() {
    ...
    container.style["height"] = "1px";
    document.body.appendChild(container);
    deployJavaWrapper.exec(
      "runApplet",
      [
        { id:"SocketJSApplet", code:"SocketJS", width:1, height:1 },
        { jnlp_href: "SocketJS_applet.jnlp", cache_option: "no" },
        "1.6"
      ]
    );
    this.install = function() {};
  },
  _conn: function(id, success) {
    if (typeof this.sockets[id] === 'undefined') return;
    this.sockets[id].connect(id, success);
  },
  _recv: function(id, line) {
    // this._log("_recv: " + id + ": " + line);
    if (typeof this.sockets[id] === 'undefined')
      this.engine.disconnect(id);
    else
      this.sockets[id].data(line);
  },
  _closed: function(id) {
    // this._log("_closed: " + id);
    if (typeof this.sockets[id] === 'undefined' || typeof this.sockets[id].disconnect !== 'function') return;
    this.sockets[id].disconnect();
  },
  _init: function() {
    this.engine = document.getElementById('SocketJSApplet');
    if (this.engine === null) {
      setTimeout(this._init, 100);
    } else {
      this.ready = true;
      this.queued.forEach(function(val, i, arr) {
        this.connect(val);
      }, this);
    }
  },
